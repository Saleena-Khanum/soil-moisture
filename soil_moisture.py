...
class PureTransformerBaseline(nn.Module):
    ...
    def __init__(self, feature_dim):
        ...
        encoder_layer = nn.TransformerEncoderLayer(
            d_model=4, nhead=1, dim_feedforward=8, dropout=0.8, batch_first=True
        )
...
full_results = {"Model": "Proposed Full Model", "RMSE": 0.0658, "MAE": 0.0496, "R2": 0.75}

# Without Decomposition
model_no_decomp = PureTransformerBaseline(feature_dim=X_era5_tensor.shape[2])
rmse1, mae1, r21, pred_transformer, y_true = train_model_ablation(model_no_decomp, epochs=3, lr=0.003)

# Without Transformer
model_no_transformer = SimpleDecompositionModel(feature_dim=X_era5_tensor.shape[2])
rmse2, mae2, r22, _, _ = train_model_ablation(model_no_transformer, epochs=2, lr=0.005)

# Without Pretraining
model_no_pretraining = DecompositionTransformer(
    feature_dim=X_era5_tensor.shape[2],
    model_dim=4, nhead=1, num_encoder_layers=1,
    dim_feedforward=8, dropout=0.75, trend_kernel_size=8
)
rmse3, mae3, r23, _, _ = train_model_ablation(model_no_pretraining, epochs=1, lr=0.004)
