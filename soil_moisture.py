...
class PureTransformerBaseline(nn.Module):
    def __init__(self, feature_dim, model_dim=128, nhead=8,
                 num_encoder_layers=3, dim_feedforward=256, dropout=0.4):
        super().__init__()
        self.input_embedding = nn.Linear(feature_dim, model_dim)
        self.dropout_layer = nn.Dropout(dropout)
        encoder_layers = [
...
class SimpleDecompositionModel(nn.Module):
    def __init__(self, feature_dim, model_dim=128, dropout=0.4,
                 trend_kernel_size=25):
        super().__init__()
        self.input_embedding = nn.Linear(feature_dim, model_dim)
    ...
    def forward(self, x):
        ...
        trend_pred = self.trend_linear(trend[:, -1, :])
...
def run_ablation_experiment(model_class, pretrain, model_kwargs, name):
    ...
    # Instantiate with correct dropout
    model_kwargs['dropout'] = 0.1 if pretrain else 0.4
    ...
    if pretrain:
        ...
        # Re-instantiate for fine-tuning
    ...
    lr = 0.0001 if pretrain else 0.001
    epochs = 30
    opt_ft = optim.Adam(mdl.parameters(), lr=lr)
