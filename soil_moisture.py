...
This script performs end-to-end soil moisture prediction including:
  ...
  16. Wasserstein distance analysis
...
for i, val_location in enumerate(unique_locations):
    # Seed each fold for reproducibility
    ...
    optimizer_fold = optim.Adam(model_fold.parameters(), lr=0.0001)
...
# Chronological split as described in the paper (not random)
train_dataset = torch.utils.data.Subset(era5_dataset, range(0, train_size))
val_dataset = torch.utils.data.Subset(era5_dataset, range(train_size, len(era5_dataset)))

batch_size = 64
train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=False)  # Chronological order for time series
val_loader = DataLoader(val_dataset, batch_size=batch_size, shuffle=False)
...
# --- Freeze pretrained backbone; train only the prediction head ---
# Freeze decomposition layer
for param in model_fine_tune.decomposition.parameters():
    param.requires_grad = False
# Freeze transformer encoder layers
for param in model_fine_tune.transformer_encoder.parameters():
    param.requires_grad = False
# Freeze trend and residual projection layers
for param in model_fine_tune.trend_linear.parameters():
    param.requires_grad = False
for param in model_fine_tune.residual_output.parameters():
    param.requires_grad = False

trainable_params = sum(p.numel() for p in model_fine_tune.parameters() if p.requires_grad)
total_params = sum(p.numel() for p in model_fine_tune.parameters())
...
optimizer = optim.AdamW(
    filter(lambda p: p.requires_grad, model_fine_tune.parameters()),
    lr=fine_tune_lr,
    weight_decay=0.01
)
...
for epoch in range(num_epochs):
    ...
    if epoch_val_loss < best_val_loss:
        ...
        patience_counter = 0
        best_model_state = {k: v.clone() for k, v in model_fine_tune.state_dict().items()}
    else:
        patience_counter += 1
...
if best_model_state is not None:
    model_fine_tune.load_state_dict(best_model_state)
...
# Ablation dataset
era5_dataset_abl = TensorDataset(X_era5_tensor, y_era5_tensor)

# Chronological split
train_size_abl = int(0.8 * len(era5_dataset_abl))
train_dataset_abl = torch.utils.data.Subset(era5_dataset_abl, range(0, train_size_abl))
val_dataset_abl = torch.utils.data.Subset(era5_dataset_abl, range(train_size_abl, len(era5_dataset_abl)))

train_loader_abl = DataLoader(train_dataset_abl, batch_size=128, shuffle=False)
val_loader_abl = DataLoader(val_dataset_abl, batch_size=128, shuffle=False)

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")


# --- Without Decomposition (Paper-faithful: full-sized Transformer, no decomposition) ---

class PureTransformerBaseline(nn.Module):
    """Transformer-only baseline matching paper dimensions but without decomposition."""

    def __init__(self, feature_dim, model_dim=128, nhead=8, num_encoder_layers=3,
...
# --- Without Transformer (Paper-faithful: decomposition + linear, no Transformer) ---

class SimpleDecompositionModel(nn.Module):
    """Decomposition-only baseline matching paper dimensions but without Transformer."""

    def __init__(self, feature_dim, model_dim=128, dropout=0.4, trend_kernel_size=25):
        super().__init__()
        self.embedding = nn.Linear(feature_dim, model_dim)
        self.decomposition = DecompositionLayer(kernel_size=trend_kernel_size)
        self.dropout = nn.Dropout(dropout)
        self.trend_linear = nn.Linear(model_dim, 1)
        self.residual_linear = nn.Linear(model_dim, 1)
        self.fc_out = nn.Linear(2, 1)
    ...
    def forward(self, x):
        ...
        trend_out = self.trend_linear(trend[:, -1, :])
        residual_out = self.residual_linear(residual[:, -1, :])
        combined = torch.cat((trend_out, residual_out), dim=-1)
...
def train_model_ablation(mdl, epochs, lr):
    mdl = mdl.to(device)
    ...
    for epoch in range(epochs):
        mdl.train()
        for inputs, targets in train_loader_abl:
            inputs = inputs.to(device)
            targets = targets.to(device)
            opt.zero_grad()
            outputs = mdl(inputs)
...
# --- Full Model: Evaluate the actual fine-tuned model on the ablation val set (computed, not hard-coded) ---
print("Evaluating fine-tuned full model on ablation validation set...")
model_full_abl = DecompositionTransformer(
    feature_dim=X_era5_tensor.shape[2],
    model_dim=128, nhead=8, num_encoder_layers=3,
    dim_feedforward=256, dropout=0.4, trend_kernel_size=25
).to(device)
model_full_abl.load_state_dict(torch.load(resolve_path("fine_tuned_soil_model.pth"), map_location=device))
model_full_abl.eval()
...
with torch.no_grad():
    for inputs, targets in val_loader_abl:
        inputs = inputs.to(device)
        outputs = model_full_abl(inputs)
        full_preds.extend(outputs.squeeze().cpu().numpy())
        full_acts.extend(targets.cpu().numpy())

full_preds = np.array(full_preds)
full_acts = np.array(full_acts)

full_preds_orig = era5_target_scaler.inverse_transform(full_preds.reshape(-1, 1)).flatten()
full_acts_orig = era5_target_scaler.inverse_transform(full_acts.reshape(-1, 1)).flatten()

full_rmse = np.sqrt(mean_squared_error(full_acts_orig, full_preds_orig))
full_mae = mean_absolute_error(full_acts_orig, full_preds_orig)
full_r2 = r2_score(full_acts_orig, full_preds_orig)

full_results = {"Model": "Proposed Full Model", "RMSE": full_rmse, "MAE": full_mae, "R2": full_r2}
print(f"Full Model - RMSE: {full_rmse:.4f}, MAE: {full_mae:.4f}, R2: {full_r2:.4f}")

# Without Decomposition
print("Training 'Without Decomposition' ablation...")
torch.manual_seed(SEED)
np.random.seed(SEED)
model_no_decomp = PureTransformerBaseline(feature_dim=X_era5_tensor.shape[2])
rmse1, mae1, r21, pred_transformer, y_true = train_model_ablation(model_no_decomp, epochs=ablation_epochs, lr=ablation_lr)
print(f"Without Decomposition - RMSE: {rmse1:.4f}, MAE: {mae1:.4f}, R2: {r21:.4f}")

# Without Transformer
print("Training 'Without Transformer' ablation...")
torch.manual_seed(SEED)
np.random.seed(SEED)
model_no_transformer = SimpleDecompositionModel(feature_dim=X_era5_tensor.shape[2])
rmse2, mae2, r22, _, _ = train_model_ablation(model_no_transformer, epochs=ablation_epochs, lr=ablation_lr)
print(f"Without Transformer - RMSE: {rmse2:.4f}, MAE: {mae2:.4f}, R2: {r22:.4f}")

# Without Pretraining (same architecture as full model, trained from scratch — no pretrained weights)
print("Training 'Without Pretraining' ablation...")
torch.manual_seed(SEED)
np.random.seed(SEED)
...
rmse3, mae3, r23, _, _ = train_model_ablation(model_no_pretraining, epochs=ablation_epochs, lr=ablation_lr)
print(f"Without Pretraining - RMSE: {rmse3:.4f}, MAE: {mae3:.4f}, R2: {r23:.4f}")
...
print("\n" + "=" * 60)
print("12. COMPUTATIONAL COMPLEXITY ANALYSIS")
print("=" * 60)
...
model_full = DecompositionTransformer(
    feature_dim=feature_dim_era5, model_dim=128, nhead=8,
    num_encoder_layers=3, dim_feedforward=256, dropout=0.4, trend_kernel_size=25
)
params_full = count_trainable_parameters(model_full)
print(f"\nProposed Full Model Parameters: {params_full:,}")

# Without Decomposition
model_no_decomp_cc = PureTransformerBaseline(feature_dim=feature_dim_era5)
params_nodecomp = count_trainable_parameters(model_no_decomp_cc)
print(f"Without Decomposition Parameters: {params_nodecomp:,}")

# Without Transformer
model_no_transformer_cc = SimpleDecompositionModel(feature_dim=feature_dim_era5)
params_notrans = count_trainable_parameters(model_no_transformer_cc)
print(f"Without Transformer Parameters: {params_notrans:,}")

# Without Pretraining (same architecture as full model)
model_no_pretraining_cc = DecompositionTransformer(
    feature_dim=feature_dim_era5, model_dim=128, nhead=8,
    num_encoder_layers=3, dim_feedforward=256, dropout=0.4, trend_kernel_size=25
)
params_nopretrain = count_trainable_parameters(model_no_pretraining_cc)
print(f"Without Pretraining Parameters: {params_nopretrain:,}")

# Computational Complexity Table
complexity_table = pd.DataFrame({
    "Model Configuration": [
        "Proposed Full Model", "Without Decomposition",
        "Without Transformer", "Without Pretraining"
    ],
    "Trainable Parameters": [params_full, params_nodecomp, params_notrans, params_nopretrain]
})
...
complexity_table.to_csv(resolve_path("computational_complexity_parameters.csv"), index=False)
...
print("\n" + "=" * 60)
print("13. STATISTICAL SIGNIFICANCE TESTING")
print("=" * 60)
...
# ############################################################
# 14. WASSERSTEIN DISTANCE ANALYSIS
# ############################################################

print("\n" + "=" * 60)
print("14. WASSERSTEIN DISTANCE ANALYSIS")
print("=" * 60)
